...
EXPORT_C CFileLineReader* CFileLineReader::DuplicateLC(RFile& aFile)
    {
...
EXPORT_C CFileLineReader* CFileLineReader::DuplicateL(RFile& aFile)
	{
...
void CFileLineReader::ConstructL()
	...
	else if(sig[0] == 0xFF && sig[1] == 0xFE && sig[2] == 0 && sig[3] == 0) //UTF32 LE
	  	{
	    User::Leave(KErrNotSupported); 
	    TInt pos = 4;
	    iFile.Seek(ESeekStart, pos);
	   	}
	else if(sig[0] == 0xFF && sig[1] == 0xFE) //UTF16 LE
	  	{
	    iReader = new (ELeave) CFileLineReaderUtf16LE(iFile);
	    TInt pos = 2;
	    iFile.Seek(ESeekStart, pos);
	   	}
	else if(sig[0] == 0xFE && sig[1] == 0xFF) //UTF16 BE
	  	{
	    iReader = new (ELeave) CFileLineReaderUtf16BE(iFile);
	    TInt pos = 2;
	    iFile.Seek(ESeekStart, pos);
	   	}
	else if(sig[0] == 0xEF && sig[1] == 0xBB && sig[2] == 0xBF) //UTF8
	  	{
...
EXPORT_C CFileLineReader* CFileLineReader::NewLC(RFs& aFs, const TDesC& aFileName)
    {
    CFileLineReader* n = new (ELeave) CFileLineReader();
    CleanupStack::PushL(n);
...
EXPORT_C CFileLineReader* CFileLineReader::NewL(RFs& aFs, const TDesC& aFileName)
    {
...
EXPORT_C CFileLineReader::~CFileLineReader()
    {
    delete iReader;
    iFile.Close();
    }

EXPORT_C  TPtrC CFileLineReader::Current() const
    {
    return iReader->Buffer().Mid(iCursor, iSize);
    }


EXPORT_C TBool CFileLineReader::NextL()
    {
    for(;;)
        {
        TInt sparecursor = -1;
...
CFileLineReaderBase::CFileLineReaderBase(RFile& aFile)	: iFile(aFile)
	{
	}
...
CFileLineReaderAscii::CFileLineReaderAscii(RFile& aFile)	: CFileLineReaderBase(aFile)
	{
	}	
	
CFileLineReaderUtf16LE::CFileLineReaderUtf16LE(RFile& aFile)	: CFileLineReaderBase(aFile)
	{
	}
	
CFileLineReaderUtf16BE::CFileLineReaderUtf16BE(RFile& aFile)	: CFileLineReaderBase(aFile)
	{
	}	   

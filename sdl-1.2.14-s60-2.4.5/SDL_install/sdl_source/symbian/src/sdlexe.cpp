...
void CSDLAppUi::ConstructL()
     ...
     iMbmFile.Create(name);
 	
	//TEntry e;
	//User::LeaveIfError(iEikonEnv->FsSession().Entry(name, e));
	
	iCBmp = iEikonEnv->CreateBitmapL(iMbmFile, EMbmSdlexePointer);
	iAlpha = iEikonEnv->CreateBitmapL(iMbmFile, EMbmSdlexePointeralpha);	
	iCBmpMove = iEikonEnv->CreateBitmapL(iMbmFile, EMbmSdlexePointer2);
...
TBool CSDLAppUi::ProcessCommandParametersL(CApaCommandLine &aCommandLine)
 ...
 void CSDLAppUi::StartL()	
    ...
    if(gSDLClass.AppFlags() & SDLEnv::EViewFPS)
        {
        CFbsBitmap* bmp = iEikonEnv->CreateBitmapL(iMbmFile, EMbmSdlexeNumber);
        iFpsView = new (ELeave) CFpsView(TPoint(150, 0), bmp);
        iFpsView->ConstructL();
        iSdl->AppendOverlay(*iFpsView , 10);
        }
